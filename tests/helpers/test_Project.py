...
def create_project():
    project = Project(
        {"app_id": "test-project", "name": "TestProject", "app_type": ""},
        name="TestProject",
        architecture=[],
        user_stories=[],
    )
        ...
        for file_name in values:
            if file_name.endswith("server.js"):
                for file_path in values:
                    ...
                    assert (
                        absolute_path == "/Users/zvonimirsabljic/Development/copilot/pilot/server.js"
            ...
            if file_name.endswith("server.js"):
                for file_path in values:
                    ...
                    expected_absolute_path = (
                        project.root_path + ("" if expected_path == "" else "/" + expected_path) + "/server.js"
                    )
        ...
        for file_name in values:
            if file_name.endswith("server.js"):
                for file_path in values:
                    ...
                    #     print(f'file_path: {file_path}, file_name: {file_name}')
                    assert (
                        absolute_path == project.root_path + "/server.js"
                    ), f"file_path: {file_path}, file_name: {file_name}"
