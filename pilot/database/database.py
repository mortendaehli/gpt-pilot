...
from pilot.utils.style import color_red, color_yellow
...
def get_created_apps_with_steps():
    ...
    for app in apps:
        ...
        app["development_steps"] = [
            {k: v for k, v in dev_step.items() if k in {"id", "created_at"}} for dev_step in app["development_steps"]
        ]
    return apps
...
def get_saved_development_step(project):
    development_step = get_db_model_from_hash_id(
        DevelopmentSteps, project.args["app_id"], project.checkpoints["last_development_step"], project.current_step
    )
    return development_step
...
def get_saved_command_run(project, command):
    ...
    command_run = get_db_model_from_hash_id(
        CommandRuns, project.args["app_id"], project.checkpoints["last_command_run"], project.current_step
    )
    return command_run
...
def get_saved_user_input(project, query):
    ...
    user_input = get_db_model_from_hash_id(
        UserInputs, project.args["app_id"], project.checkpoints["last_user_input"], project.current_step
    )
    return user_input
...
def save_file_description(project, path, name, description):
    (
        ...
        .on_conflict(conflict_target=[File.app, File.name, File.path], preserve=[], update={"description": description})
        .execute()
    )
