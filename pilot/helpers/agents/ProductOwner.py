...
from pilot.database.database import get_app, get_progress_steps, save_app, save_progress
...
from pilot.prompts.prompts import (
    ask_for_app_type,
    ask_for_main_app_definition,
    ask_user,
    generate_messages_from_description,
    get_additional_info_from_openai,
    get_prompt,
)
from pilot.utils.files import setup_workspace
from pilot.utils.style import color_green_bold
from pilot.utils.utils import clean_filename, generate_app_data, should_execute_step, step_already_finished
...
class ProductOwner(Agent):
    ...
    def ask_clarifying_questions(self, main_prompt: str):
        instructions = generate_messages_from_description(
            main_prompt, self.project.args["app_type"], self.project.args["name"]
        )
    ...
    def generate_project_summary(self, high_level_messages: list[dict]):
        ...
        convo_project_description.messages.append(
            {"role": "system", "content": get_prompt("components/summary_instructions.prompt")}
        )
        return convo_project_description.send_message(
