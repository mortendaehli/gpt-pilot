...
from pilot.const.function_calls import (
    COMMAND_TO_RUN,
    EXECUTE_COMMANDS,
...
from pilot.database.database import get_progress_steps, save_progress, update_app_status
...
from pilot.helpers.cli import execute_command_and_check_cli_response, run_command_until_success, running_processes
...
class Developer(Agent):
        ...
        if not self.project.finished:
            ...
            if self.project.skip_steps is None:
                self.project.skip_steps = (
                    False
                    if ("skip_until_dev_step" in self.project.args and self.project.args["skip_until_dev_step"] == "0")
                    else True
                )
    ...
    def step_command_run(self, convo, step, i, success_with_cli_response=False):
        ...
        # TODO END
        additional_message = (
            "Let's start with the step #0:\n"
            if i == 0
            else f'So far, steps { ", ".join(f"#{j}" for j in range(i+1)) } are finished so let\'s do step #{i + 1} now.\n'
        )
        ...
        while True:
            ...
            if response["user_input"] == "continue":
                ...
                response["success"] = self.debugger.debug(
                    convo, user_input=response["user_input"], issue_description=step["human_intervention_description"]
                )
                # TODO add review
    ...
    def dev_help_needed(self, step):
        ...
        while answer.lower() != "continue":
            ...
            answer = styled_text(self.project, WHEN_USER_DONE)
    ...
    def install_technology(self, technology):
        ...
        llm_response = self.convo_os_specific_tech.send_message(
            ...
            {"technology": technology},
            {
                ...
                "functions": {"execute_command": lambda command, timeout: (command, timeout)},
            },
        )
